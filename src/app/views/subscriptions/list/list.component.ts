...
import { SubscriptionItem } from 'src/app/models/subscription/subscription-item';
...
export class SubscriptionsListComponent implements OnInit, OnDestroy {
  ...
  subscriptionTable: Table<SubscriptionItem, DateFilter>;
  urlFilter: CommonUrlFilter<DateFilter>;
  pageIndex: number;
  filterChanges: Observable<DateFilter>;
  subscriptionsList: Observable<SubscriptionItem[]>;
  limit = PAGE_SIZE;
  offset = 0;
  currentOffset = 0;
  q: string;
  ...
  ngOnDestroy(): void {
    ...
    this.subscriptionTable.disconnect();
  }
  ...
  getFile() {
    this.subscriptionsService.getFile({
      q: this.q,
      offset: null,
      limit: null,
  ...
  ngOnInit(): void {
    ...
    this.subscriptionTable = new Table(this.subscriptionsService, this.filterChanges);
    ...
    this.subscriptionsList = this.subscriptionTable.connect();
